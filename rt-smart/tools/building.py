...
def BuildApplication(TARGET, SConscriptFile, usr_root = None, **parameters):
    ...
    env['CXXFLAGS'] = env['CXXFLAGS'].replace('-Werror -Wall', '')
    ...
    # # 添加静态库路径
    # env.AppendUnique(LIBPATH=[os.path.join(USR_ROOT, 'sdk', 'modbus', 'lib')])

    # # 链接静态库
    # env.AppendUnique(LIBS=['modbus'])
    # env['LINKCOMSTR'] = 'Linking $TARGET'
    # handle local group
    objs = [obj for obj in objs if hasattr(obj, 'name')]  # 过滤无效项
    ...
    def strip_and_move_files(target, source, env):
        target_path = str(target[0])
        release_dir = os.path.join(usr_root, "release")
        elf_dir = os.path.join(release_dir, "elf")
        os.makedirs(elf_dir, exist_ok=True)
        stripped_elf_path = os.path.join(elf_dir, os.path.basename(target_path))
        strip_command = f"arm-linux-musleabi-strip  {target_path} -o {stripped_elf_path}"
        env.Execute(strip_command)
        print(f"Stripped and moved ELF to {stripped_elf_path}")

        fatdisk_exe = os.path.join(usr_root, "root", "fatdisk.exe")
        sd_bin = os.path.join(usr_root, "sd.bin")
        target_dir = os.path.join(usr_root, "..", "kernel", "bsp", "qemu-vexpress-a9")

        # 检查fatdisk.exe是否存在
        if not os.path.exists(fatdisk_exe):
            print(f"Error: fatdisk.exe not found in {fatdisk_exe}")
            return

        # 运行fatdisk.exe
        print("Running fatdisk.exe to generate sd.bin...")
        try:
            subprocess.run(fatdisk_exe, check=True)
        except subprocess.CalledProcessError as e:
            print(f"Error: fatdisk.exe failed with error {e.returncode}")
            return

        # 检查sd.bin是否生成
        max_retries = 5
        retry_count = 0
        while not os.path.exists(sd_bin) and retry_count < max_retries:
            retry_count += 1
            time.sleep(1)

        if not os.path.exists(sd_bin):
            print(f"Error: sd.bin was not generated by fatdisk.exe after {max_retries} attempts!")
            print(f"Expected location: {sd_bin}")
            return

        # 复制sd.bin到目标目录
        print(f"Copying sd.bin to {target_dir}...")
        try:
            shutil.copy(sd_bin, target_dir)
            print(f"Success! sd.bin has been copied to {target_dir}")
        except Exception as e:
            print(f"Error: Failed to copy sd.bin to {target_dir}: {str(e)}")

    env.AddPostAction(target, Action(strip_and_move_files, "Stripping and moving files"))
...
def BuildLibrary(TARGET, SConscriptFile, usr_root = None, **parameters):
    ...
    if 'build' in parameters:
        ...
        target_vdir = 'build/' + TARGET
    headers=[]
    result = SConscript(SConscriptFile, variant_dir=target_vdir, duplicate=0)
    if isinstance(result, tuple):  # 如果返回 ('group', 'headers')
        objs, headers = result
    else: 
        objs = result
    ...
    if len(headers)==0:
        return None
    objs.extend(SConscript(USR_ROOT + '/sdk/rt-thread/SConscript', variant_dir=target_vdir + '/sdk', duplicate=0))
    ...
    # 定义目标目录
    include_dir = os.path.join('sdk', TARGET, 'include')  # 头文件目标目录
    lib_dir = os.path.join('sdk', TARGET, 'lib')  # 静态库目标目录

    # 创建目标目录（如果它们不存在）
    env.Command(include_dir, [], Mkdir(include_dir))
    env.Command(lib_dir, [], Mkdir(lib_dir))
    ...
    def strip_and_move_files(target, source, env):
        ...
        stripped_lib_path = os.path.join(lib_dir, os.path.basename(target_path))
        strip_command = f"arm-linux-musleabi-strip --strip-debug {target_path} -o {stripped_lib_path}"
        env.Execute(strip_command)
        print(f"Stripped and moved library to {stripped_lib_path}")

    env.AddPostAction(target, Action(strip_and_move_files, "Stripping and moving files"))

 # ===== 自动生成 SConscript 文件 =====
    def generate_sconscript(target_name):
        sconscript_content = f"""from building import *
...
group = DefineGroup('sdk_{target_name}', src, depend = [''], CPPPATH = CPPPATH, 
        LIBPATH = LIBPATH, LIBS = LIBS)

Return('group')
"""
        sconscript_path = os.path.join('sdk', target_name, 'SConscript')
        os.makedirs(os.path.dirname(sconscript_path), exist_ok=True)
        with open(sconscript_path, 'w') as f:
            f.write(sconscript_content)
        print(f"Generated SConscript at: {sconscript_path}")

    generate_sconscript(TARGET)
    return target


def clean_library(target):
    lib_path = os.path.join('build', target)
    if os.path.exists(lib_path):
        print("Cleaning library: "+lib_path)
        # 遍历目录并删除文件
        for root, dirs, files in os.walk(lib_path, topdown=False):
            ...
            for name in dirs:
                ...
                os.rmdir(dir_path)
        # 删除根目录
        print("Deleting root directory: "+lib_path)
        os.rmdir(lib_path)
    else:
        print("Library directory does not exist: "+lib_path)
